...
![image-20220707200915836](.images_g12_数据库其他调优策略/image-20220707200915836.png)

如何确定呢？一般情况下，有如下几种方式：

![image-20220707201133424](.images_g12_数据库其他调优策略/image-20220707201133424.png)
...
![image-20220707201443229](.images_g12_数据库其他调优策略/image-20220707201443229.png)

#### 第2步：优化表设计

![image-20220707201617799](.images_g12_数据库其他调优策略/image-20220707201617799.png)

#### 第3步：优化逻辑查询

![image-20220707202059972](.images_g12_数据库其他调优策略/image-20220707202059972.png)

#### 第4步：优化物理查询

物理查询优化是在确定了逻辑查询优化之后，采用物理优化技术（比如索引等），通过计算代价模型对 各种可能的访问路径进行估算，从而找到执行方式中代价最小的作为执行计划。**在这个部分中，我们需要掌握的重点是对索引的创建和使用。**

![image-20220707202156660](.images_g12_数据库其他调优策略/image-20220707202156660.png)
...
![image-20220707202436467](.images_g12_数据库其他调优策略/image-20220707202436467.png)

#### 第6步：库级优化

![image-20220707202555506](.images_g12_数据库其他调优策略/image-20220707202555506.png)

![image-20220707202732911](.images_g12_数据库其他调优策略/image-20220707202732911.png)

![image-20220707203538155](.images_g12_数据库其他调优策略/image-20220707203538155.png)

![image-20220707203607993](.images_g12_数据库其他调优策略/image-20220707203607993.png)

> 但需要注意的是，分拆在提升数据库性能的同时，也会增加维护和使用成本。

## 2. 优化MySQL服务器

![image-20220707203818987](.images_g12_数据库其他调优策略/image-20220707203818987.png)
...
![image-20220707204403406](.images_g12_数据库其他调优策略/image-20220707204403406.png)
...
![image-20220707210351452](.images_g12_数据库其他调优策略/image-20220707210351452.png)

**(1) 调整系统参数 InnoDB_flush_log_at_trx_commit**

![image-20220707210447501](.images_g12_数据库其他调优策略/image-20220707210447501.png)

**(2)  调整系统参数 InnoDB_buffer_pool_size**

![image-20220707210555848](.images_g12_数据库其他调优策略/image-20220707210555848.png)

**(3) 调整系统参数 InnoDB_buffer_pool_instances**

![image-20220707210720394](.images_g12_数据库其他调优策略/image-20220707210720394.png)

## 3. 优化数据库结构

![image-20220707211709553](.images_g12_数据库其他调优策略/image-20220707211709553.png)

### 3.1 拆分表：冷热数据分离

![image-20220707211802756](.images_g12_数据库其他调优策略/image-20220707211802756.png)
...
![image-20220707212800544](.images_g12_数据库其他调优策略/image-20220707212800544.png)
...
![image-20220707213524137](.images_g12_数据库其他调优策略/image-20220707213524137.png)
...
![image-20220707214640374](.images_g12_数据库其他调优策略/image-20220707214640374.png)
...
![image-20220707215305640](.images_g12_数据库其他调优策略/image-20220707215305640.png)

**② 禁用唯一性检查**

![image-20220707215356893](.images_g12_数据库其他调优策略/image-20220707215356893.png)
...
![image-20220707220034534](.images_g12_数据库其他调优策略/image-20220707220034534.png)

**③ 禁止自动提交**

![image-20220707220131891](.images_g12_数据库其他调优策略/image-20220707220131891.png)

### 3.6 使用非空约束

![image-20220707220157606](.images_g12_数据库其他调优策略/image-20220707220157606.png)
...
![image-20220707221707254](.images_g12_数据库其他调优策略/image-20220707221707254.png)
...
![image-20220707221901664](.images_g12_数据库其他调优策略/image-20220707221901664.png)
...
![image-20220707222156765](.images_g12_数据库其他调优策略/image-20220707222156765.png)

**方式二：使用mysqlcheck命令**

![image-20220707222305302](.images_g12_数据库其他调优策略/image-20220707222305302.png)
...
![image-20220707222606097](.images_g12_数据库其他调优策略/image-20220707222606097.png)

- 双主双从模式：

![image-20220707222623485](.images_g12_数据库其他调优策略/image-20220707222623485.png)

### 4.3 垂直拆分

当数据量级达到 `千万级` 以上时，有时候我们需要把一个数据库切成多份，放到不同的数据库服务器上， 减少对单一数据库服务器的访问压力。

![image-20220707222648112](.images_g12_数据库其他调优策略/image-20220707222648112.png)

- 如果数据库的数据表过多，可以采用`垂直分库`的方式，将关联的数据库部署在同一个数据库上。
- 如果数据库中的列过多，可以采用`垂直分表`的方式，将一张数据表分拆成多张数据表，把经常一起使用的列放在同一张表里。

![image-20220707222910740](.images_g12_数据库其他调优策略/image-20220707222910740.png)
...
![image-20220707222954304](.images_g12_数据库其他调优策略/image-20220707222954304.png)

![image-20220707222739120](.images_g12_数据库其他调优策略/image-20220707222739120.png)

![image-20220707223024163](.images_g12_数据库其他调优策略/image-20220707223024163.png)
...
![image-20220707223246684](.images_g12_数据库其他调优策略/image-20220707223246684.png)

![image-20220707223349879](.images_g12_数据库其他调优策略/image-20220707223349879.png)

### 5.3 MySQL 8.0新特性：隐藏索引对调优的帮助

![image-20220707223420496](.images_g12_数据库其他调优策略/image-20220707223420496.png)