...
![image-20220714140441064](.images_g16_多版本并发控制_mvcc/image-20220714140441064.png)

![image-20220714140510426](.images_g16_多版本并发控制_mvcc/image-20220714140510426.png)

![image-20220714140541555](.images_g16_多版本并发控制_mvcc/image-20220714140541555.png)
...
![image-20220714140716427](.images_g16_多版本并发控制_mvcc/image-20220714140716427.png)

假设插入该记录的`事务id`为`8`，那么此刻该条记录的示意图如下所示：

![image-20220714140801595](.images_g16_多版本并发控制_mvcc/image-20220714140801595.png)

> insert undo只在事务回滚时起作用，当事务提交后，该类型的undo日志就没用了，它占用的Undo Log Segment也会被系统回收（也就是该undo日志占用的Undo页面链表要么被重用，要么被释放）。

假设之后两个事务id分别为 `10` 、 `20` 的事务对这条记录进行`UPDATE` 操作，操作流程如下：

![image-20220714140846658](.images_g16_多版本并发控制_mvcc/image-20220714140846658.png)

![image-20220714140908661](.images_g16_多版本并发控制_mvcc/image-20220714140908661.png)

每次对记录进行改动，都会记录一条undo日志，每条undo日志也都有一个 `roll_pointer` 属性 （ `INSERT` 操作对应的undo日志没有该属性，因为该记录并没有更早的版本），可以将这些 `undo日志` 都连起来，串成一个链表：

![image-20220714141012874](.images_g16_多版本并发控制_mvcc/image-20220714141012874.png)
...
![image-20220714141154235](.images_g16_多版本并发控制_mvcc/image-20220714141154235.png)
...
![image-20220714142254768](.images_g16_多版本并发控制_mvcc/image-20220714142254768.png)
...
![image-20220715130639408](.images_g16_多版本并发控制_mvcc/image-20220715130639408.png)

在隔离级别为读已提交（Read Committed）时，一个事务中的每一次 SELECT 查询都会重新获取一次 Read View。

如表所示：

![image-20220715130843147](.images_g16_多版本并发控制_mvcc/image-20220715130843147.png)

> 注意，此时同样的查询语句都会重新获取一次 Read View，这时如果 Read View 不同，就可能产生不可重复读或者幻读的情况。

当隔离级别为可重复读的时候，就避免了不可重复读，这是因为一个事务只在第一次 SELECT 的时候会获取一次 Read View，而后面所有的 SELECT 都会复用这个 Read View，如下表所示：

![image-20220715130916437](.images_g16_多版本并发控制_mvcc/image-20220715130916437.png)

## 5. 举例说明

![image-20220715131200077](.images_g16_多版本并发控制_mvcc/image-20220715131200077.png)
...
![image-20220715133640655](.images_g16_多版本并发控制_mvcc/image-20220715133640655.png)
...
![image-20220715134540737](.images_g16_多版本并发控制_mvcc/image-20220715134540737.png)
...
![image-20220715134839081](.images_g16_多版本并发控制_mvcc/image-20220715134839081.png)
...
![image-20220715135017000](.images_g16_多版本并发控制_mvcc/image-20220715135017000.png)

![image-20220715135143939](.images_g16_多版本并发控制_mvcc/image-20220715135143939.png)
...
![image-20220715140006061](.images_g16_多版本并发控制_mvcc/image-20220715140006061.png)
...
![image-20220715140155744](.images_g16_多版本并发控制_mvcc/image-20220715140155744.png)
...
![image-20220715140354217](.images_g16_多版本并发控制_mvcc/image-20220715140354217.png)
...
![image-20220715140555172](.images_g16_多版本并发控制_mvcc/image-20220715140555172.png)

![image-20220715140620328](.images_g16_多版本并发控制_mvcc/image-20220715140620328.png)
...
![image-20220715141002035](.images_g16_多版本并发控制_mvcc/image-20220715141002035.png)
...
![image-20220715141208667](.images_g16_多版本并发控制_mvcc/image-20220715141208667.png)
...
![image-20220715141243993](.images_g16_多版本并发控制_mvcc/image-20220715141243993.png)
...
![image-20220715141413135](.images_g16_多版本并发控制_mvcc/image-20220715141413135.png)

通过MVCC我们可以解决：

![image-20220715141515370](.images_g16_多版本并发控制_mvcc/image-20220715141515370.png)